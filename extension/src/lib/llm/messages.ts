...
type ContentPart =
    ...
    | { type: "file"; payload: FileFormat["payload"] };
...
function toToolResult(msg: Extract<ExtensionMessage, { type: "action.result" }>): ToolResultContent {
    return {
        type: "tool-result",
        toolCallId: msg.id,
        toolName: msg.name,
        ...
        switch (msg.type) {
            ...
            case "screenshot":
                if (mode === "t2" && msg.image)
                    appendScreenshotMessage(messages, msg, screenshotState);
                break;
        }
    }
    return messages;
...
export function buildT1Messages(promptText: string, promptFiles: FileFormat[], conversationMessages: Message[]): ExtensionMessage[] {
...
export function buildT2Messages(task: string, promptFiles: FileFormat[], conversationMessages: Message[]): ExtensionMessage[] {
    const { previousTask } = buildPreviousContext(conversationMessages);
    return [
        ...previousTask,
        { type: "prompt", content: toExtensionContentParts(task, promptFiles) },
    ];
}
