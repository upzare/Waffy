...
function toolCallToRecord(toolCalls: Array<{ toolCallId: string; toolName: string; input: unknown }>): Record<string, ToolCall> {
...
export async function* runStream(options: StreamOptions): AsyncGenerator<StreamEvent> {
    ...
    try {
        const result = streamText({
            ...
            tools: Object.keys(tools).length > 0 ? (tools as Parameters<typeof streamText>[0]["tools"]) : undefined,
            abortSignal,
        });
        ...
        if (completedToolCalls.length > 0) {
            ...
            for (const tc of completedToolCalls) {
                ...
                if (mode === "t2" && screenshotState.image && input.x !== undefined && input.y !== undefined) {
            ...
            if (mode === "t2" && session && textResponse) {
                try {
                    step = await generateStepLabel(
                        session.previousReasoning,
                        textResponse,
                        action,
                        settings
                    );
